...
void http_conn::initmysql_result(connection_pool *connPool) {
	...
	//从表中检索完整的结果集
	MYSQL_RES *reslut = mysql_store_result(mysql);

	//返回结果集的列数
	int num_fileds = mysql_num_fields(reslut);

	//返回所有字段的结构数目
	int num_fileds = mysql_fetch_fields(reslut);

	//从结果集中获取下一行，将对应的用户名和密码存入map
	while (MYSQL_ROW row = mysql_fetch_row(reslut)) {
		string temp1(row[0]);
		string temp2(row[1]);
		users[temp1]=temp2;
	}
...
void http_conn::init (int sockfd,const sockaddr_in &addr,char *root,int TRIGMode
	...
	//当浏览器出现连接重置时，可能是网站根目录出错或http响应格式出错或者是访问的文件的内容为空
	doc_root = root;
	m_TRIGMode TRIGMode;
...
//从状态机，用于分析出一行内容
//返回值为行为的读取状态，有LINE_OK，LINE_BAD，LINE_OPEN
...
http_conn::HTTP_CODE http_conn::parse_request_line (char *text) {
	...
	*m_url++ = '\0';
	char *method = text;
	//strcasecmp用忽略大小写比较字符串,参数s1和s2字符串前n个字符
...
bool http_conn::write() {
	...
	while(1) {
		...
		if(bytes_have_send>=m_iv[0].iov_len) {
			...
			m_io[0].iov_base= m_write_buf+bytes_have_send;
			m_iv[0].iov_len = m_iv[0].iov_len-bytes_have_send;
		}
		if(bytes_to_send<=0) {
			unmap();
			modfd(m_epollfd,EPOLLIN,m_TRIGMode);
			if(m_linger) {
				init();
				return true;
			}else {
...
bool http_conn::process_write(HTTP_CODE ret) {
	switch (ret) {
		...
		case FORBIDDEN_REQUEST: {
			...
			add_headers(403,strlen(error_403_form));
